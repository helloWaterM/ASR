	...
	if (fp) {
		...
		if (!strcmp(id, "RIFF")) {
			...
			if (!strcmp(id, "WAVE")) {
				...
				spFile->pointSize = data_size / (bits_per_sample / 8);  //有多少个采样
				// 动态分配了空间，记得要释放
...
int processSpeech(WavData& speechAudio) {
	int frameLen = 8000 / 1000 * 25; // 25ms 为1帧 
	int frameStepLen = 8000 / 1000 * 10; // 10ms分一次帧 
	int frameNum = (speechAudio.pointSize - frameLen) / frameStepLen + 1;

	

	if ((speechAudio.pointSize - frameLen) % frameStepLen != 0) { //最后还有不全的帧 
		frameNum += 1;
	}
...
vector<pair<int, int> > cutFrame(vector<double>  &Energys, vector<double> & zeroCroRates){
	...
	cout <<"语音平均能量:" << meanEnergy<<endl;
	cout <<"语音平均过零率 " << meanZero << endl;
	double energyThreshold = 2.4864e+008 * 1.5;
	double zeroCroThreshold = 0.388074 ;
	
	// 使用能量和过零率划分出静音 
	vector<int> isSilence(frameNum, 0); //记录是否为静音帧 
	for(int i = 0; i < frameNum; i++){
	...
	//假设50帧中有40帧为silence, 那么就判定整段为静音段,
	...
	int silVal = 0;
	int silHead, silTail;
	bool silStarted = false; //一个静音片段是否开始 
	...
	for(int i = 1; i < frameNum - silNumFrameThre + 1; i++){
		...
		if(silVal >= silNumThreshold){ //判定为静音段的情况下 
			if(silStarted == false){
				silHead = i;
				silStarted = true;
			}
			...
			if(silStarted == true){ //前文有判定为静音段 
				silStarted = false;
				cout <<"静音段:" << silHead << " ; " << silTail<<endl; 
